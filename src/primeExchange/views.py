...
def download_to_excel(request):
    ...
    rows = doctorMaster.objects.all().values_list('doctorID',
			'doctorName',
			'doctorDesignation',
			'doctorSpecialization',
			)
...
def time(request):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			year=form.cleaned_data.get('years')
			print("month=",month)
			print("year=",year)
			token["month"]=month
			token["year"]=year
	...
	return render_to_response("primeExchange/time.html",token,context_instance=RequestContext(request))
